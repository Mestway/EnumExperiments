...
public class LeftJoinEnumerator {
    ...
    public static List<AbsLeftJoinNode> enumLeftJoinFromEC(EnumContext ec) {
        List<AbsLeftJoinNode> result = new ArrayList<>();
        List<AbsTableNode> baseTableNodes = ec.getTableNodes();
    ...
    public static List<AbsLeftJoinNode> enumLeftJoin(AbsTableNode tn1, AbsTableNode tn2) {

        List<AbsLeftJoinNode> leftJoinResult = new ArrayList<>();
        ...
        for (int keyLength = 1; keyLength <= GlobalConfig.LEFT_JOIN_KEY_LENGTH; keyLength ++) {
            ...
            for (List<Pair<String, String>> l : combinations) {
                leftJoinResult.add(new AbsLeftJoinNode(tn1, tn2, l));
            }
        }
