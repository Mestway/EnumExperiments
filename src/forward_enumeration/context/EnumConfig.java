...
public class EnumConfig {
    ...
    public EnumConfig(String fileContent) {
        ...
        this.aggrFuns = p.aggregation_functions.stream().map(
                x -> {
                    switch (x) {
                        case "min": return AbsAggrNode.AggrMin;
                        case "max": return AbsAggrNode.AggrMax;
                        case "avg": return AbsAggrNode.AggrAvg;
                        case "count": return AbsAggrNode.AggrCount;
                        case "count-distinct": return AbsAggrNode.AggrCountDistinct;
                        case "concat": return AbsAggrNode.AggrConcat;
                        case "sum": return AbsAggrNode.AggrSum;
                        case "first": return AbsAggrNode.AggrFirst;
                        default: return null;
                    }
                }
        ).filter(x ->  x != null).collect(Collectors.toList());

        if (this.aggrFuns.contains(AbsAggrNode.AggrConcat))
            aggrFuns.add(AbsAggrNode.AggrConcat2);
    }
}
