...
public class EnumContext {
    ...
    private List<AbsTableNode> parameterizedTableNodes = new ArrayList<>();
    ...
    private List<AbsTableNode> tableNodes = new ArrayList<>();
    ...
    public EnumContext(List<Table> tbs, EnumConfig c) {
        this.inputs = tbs;
        this.tableNodes = tbs.stream().map(t -> new AbsNamedTable(t)).collect(Collectors.toList());
    ...
    public void setParameterizedTables(List<AbsTableNode> tbs) { this.parameterizedTableNodes = tbs; }
    public List<AbsTableNode> getParameterizedTables() { return this.parameterizedTableNodes; }
    ...
    public void setTableNodes(List<AbsTableNode> tableNodes) { this.tableNodes = tableNodes; }
    ...
    public static List<Function<List<Value>, Value>> getAggrFuns(ValType type, List<Function<List<Value>, Value>> enabledAggrFuns) {
        List<Function<List<Value>, Value>> fs = AbsAggrNode.getAllAggrFunctions(type);
    ...
    public boolean isInputTableNode(AbsTableNode tn) {
        if (! (tn instanceof AbsNamedTable))
            return false;
        else {
            Table bj = ((AbsNamedTable) tn).getTable();
