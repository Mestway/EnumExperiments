...
public class TableRow {
    ...
    public static TableRow TableRowFromString(List<String> names, List<String> rowContent) {
    ...
    public static TableRow TableRowFromContent(List<String> names, List<Value> rowContent) {
        TableRow newRow = new TableRow();
    ...
    /**
     * Override the value at the provided index with the new one.
     * @param index
     * @param newValue
     */
    public void overrideValue(int index, Value newValue) {
        if (newValue.getValType() != this.values.get(index).getValType())
            System.out.println("[ERROR@Tablerow207] value type not compatible: "
                    + newValue.getValType() + " - " + this.values.get(index).getValType());
