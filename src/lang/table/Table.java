...
import lang.sql.datatype.CombinedVal;
import lang.sql.datatype.LazyAggrVal;
...
public class Table {
    ...
    public Table(String tableName, List<String> schema, List<List<String>> rawContent) {
        List<TableRow> rows = rawContent
                .stream()
                .map(sList -> TableRow.TableRowFromString(schema, sList))
    ...
    public boolean contentStrictEquals(Table table) {
        return this.contentEquals(table)
                && this.getContent().size() == table.getContent().size();
    }
    ...
    public Table eliminateLazyVal() {
        Table newTable = this.duplicate();
        newTable.rows = new ArrayList<>();
        for (TableRow r : this.rows) {
            TableRow template = r.duplicate();
            // eliminate all lazy values in row r and store them in template row
            for (int j = 0; j < r.values.size(); j ++) {
                if (r.getValue(j) instanceof LazyAggrVal) {
                    template.overrideValue(j, ((LazyAggrVal) r.getValue(j)).instantiateToCombinedVal());
                }
            }
            newTable.rows.add(template);
        }
        return newTable;
    }

    public Table eliminateCombinedVal() {
        if (rows.size() == 0)
            return this;

        // the index of the combined value to eliminate
        int eliminationIndex = -1;
        for (int j = 0; j < rows.get(0).getValues().size(); j ++) {
            if (rows.get(0).getValue(j) instanceof CombinedVal) {
                eliminationIndex = j;
                break;
            }
        }
        ...
        for (TableRow r : this.rows) {
            for (Value v : ((CombinedVal) r.getValue(eliminationIndex)).getComponents()) {
                TableRow template = r.duplicate();
                template.overrideValue(eliminationIndex, v);
                newTable.rows.add(template);
            }
        }

        // proceeds to the next round of combined value elimination
        return newTable.eliminateCombinedVal();
    }
}
