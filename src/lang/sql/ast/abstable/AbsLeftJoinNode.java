...
public class AbsLeftJoinNode extends AbsTableNode {
    ...
    public AbsTableNode substCoreTable(AbsTableNode leftCore, AbsTableNode rightCore) {
        ...
        AbsTableNode lrt = RenameTNWrapper.tryRename(leftCore);
        AbsTableNode rrt = RenameTNWrapper.tryRename(rightCore);
    ...
    @Override
    public void genConstraints(Context ctx, Solver solver,
                               Map<AbsTableNode, TableAttr> map) {

        // deal with children first
        this.tn1.genConstraints(ctx, solver, map);
        this.tn2.genConstraints(ctx, solver, map);
        ...
        TableAttr attr1 = map.get(this.tn1);
        TableAttr attr2 = map.get(this.tn2);

        solver.add(ctx.mkLe(r, attr1.r));
        solver.add(ctx.mkEq(c, ctx.mkSub(ctx.mkAdd(attr1.c, attr2.c), ctx.mkInt(this.joinKeys.size()))));
        solver.add(ctx.mkLe(newVal, ctx.mkAdd(attr1.newVal, attr2.newVal, ctx.mkInt(1))));
    }
