 ...
 */
public class AbsAggrNode extends AbsTableNode {
    ...
    public AbsAggrNode(AbsTableNode tn,
                       List<String> fields,
    ...
    public Table eval(Environment env) throws SQLEvalException {
        ...
        Table tbl = tn.eval(env).eliminateLazyVal().eliminateCombinedVal();
        ...
        for (int i = 0; i < tbl.getContent().size(); i ++) {
            ...
            // initialize a container to contain these nodes
            ...
            for (int k = 0; k < this.targets.size(); k ++) {
                // this is the old approach to compute aggregation result precisely
                ...
                Value tempV = new LazyAggrVal(aggregationTargetLists.get(k), targets.get(k).getValue());
                agrResultList.add(tempV);
            }
            List<Value> rowContent = new ArrayList<>();
            for (Value v : valuesForTheRow) {
                rowContent.add(v);
            }
            rowContent.addAll(agrResultList);

            TableRow newTableRow = TableRow.TableRowFromContent(resultMeta, rowContent);
    ...
    public AbsTableNode substNamedVal(ValNodeSubstBinding vnsb) {
        return new AbsAggrNode(tn.substNamedVal(vnsb), this.groupbyColumns, this.targets);
    }
    ...
    public AbsTableNode tableSubst(List<Pair<AbsTableNode,AbsTableNode>> pairs) {
        ...
        AbsTableNode core = this.tn.tableSubst(pairs);
        ...
        AbsTableNode an = new AbsAggrNode(
                core,
    ...
    public AbsTableNode substCoreTable(AbsTableNode newCore) {

        AbsTableNode rt = RenameTNWrapper.tryRename(newCore);
        ...
        AbsTableNode newTN = new AbsAggrNode(rt, newGroupByFields, newTargets);
        return newTN;
    }
    ...
    public void genConstraints(Context ctx,
        ...
        this.tn.genConstraints(ctx, solver, map);
        ...
        TableAttr attr = map.get(tn);

        solver.add(ctx.mkLe(r, attr.r));
        solver.add(ctx.mkEq(c, ctx.mkInt(groupbyColumns.size() + this.targets.size())));
        solver.add(ctx.mkLe(newVal, ctx.mkAdd(attr.r, attr.newVal)));
    }
}
