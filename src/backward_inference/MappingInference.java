...
import lang.sql.ast.filter.Filter;
...
public class MappingInference {
    ...
    public static Map<Value, Set<CellIndex>> inverseTable(Table t) {
        ...
        for (int i = 0; i < t.getContent().size(); i ++) {
            for (int j = 0; j < t.getContent().get(i).getValues().size(); j ++) {
                ...
                Set<Value> concreteValues = new HashSet<>();
                if (cellVal instanceof CombinedVal) {
                    concreteValues = ((CombinedVal) cellVal).getComponents();
                } else if (cellVal instanceof LazyAggrVal) {
                    concreteValues = ((LazyAggrVal) cellVal).instantiateToCombinedVal().getComponents();
                } else {
                    concreteValues.add(cellVal);
                }

                for (Value val : concreteValues) {
                    if (! inverseTableMap.containsKey(val))
                        inverseTableMap.put(val, new HashSet<>());
                    inverseTableMap.get(val).add(new CellIndex(i, j));
                }
            }
        }
